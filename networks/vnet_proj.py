...
class VNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=True,
                 proj_head_mode='linear', align_corners=True, is_interpolation=True, scale_factor=4.0):
        ...
        self.projection_head = ProjectionHead(proj_head_mode=proj_head_mode, n_filters_in=256)

        # classification head for last third stage of the decoder
        self.classification_head = nn.Conv3d(n_filters * 4, n_classes, kernel_size=1)
    ...
    def forward(self, input, turnoff_drop=False):
        ...
        # output of projection (segmentation) head
        projected = self.projection_head(features[-1])

        if self.is_interpolation:
            projected = F.interpolate(input=projected, scale_factor=self.scale_factor,
                                      mode='trilinear', align_corners=self.align_corners)

        # mid_feature for deep supervision
        out, mid_feature = self.decoder(features)

        # feed mid_feature into the classification head for contrastive loss
        mid_feature = self.classification_head(mid_feature)

        if turnoff_drop:
            self.has_dropout = has_dropout

        # [out, mid_feature]: list, for deep supervision
        # out: tensor, for consistency loss
        # projected: tensor, for contrastive loss
        return [out, mid_feature], projected
